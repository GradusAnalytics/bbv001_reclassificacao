...
`ID Lançamento`, é a chave verdadeira do lançamento (1..N na base bruta).
...
| Colunas | 15 | **19** (as 15 nas mesmas posições + `ID Lançamento`·`Status`·`Motivo`·`Ação Recomendada`) |
| Lançamento sem cadastro a montante | sai da base / aborta a rodada | fica na base, `Status = CADASTRO_PENDENTE`, `Conta destino` = conta de **origem** |
...
⚠️ **Advertência operacional para quem carrega no Matrix — leia antes de subir a v3.**
Como nada mais sai da base, **é o consumidor que decide o que carregar, filtrando por
`Status`**. Em Abr26 são **8.630 lançamentos / R$ 11.810.587,57** com
`Status = DESTINO_SUSPEITO` (7.859 deles / R$ 7.608.310,00 por `PREFIXO_818_CRUZADO`), cuja
`Conta destino` cruza família de prefixo **de propósito** — é a decisão D4 do dono: manter a
conta calculada e marcá-la, em vez de escondê-la. **Se ninguém filtrar, esse valor migra de
família de conta no Matrix.** Em Jun26 são 7.461 lançamentos / −R$ 1.523.293,99.
Os números completos por `Status` e por `Motivo` estão em `SCHEMA_OUTPUT.md §8.5`.
...
1. **`base_final` — 2 abas × 19 colunas, marcação por lançamento.** Todo lançamento da Base de
   ...
   = só as Classes de Valor excluídas por configuração. O `Status` tem **6 valores** (`FALHA_INTERNA`
   entrou na v3.1). Schema completo, enum de `Status` e catálogo dos 15 códigos de `Motivo`:
   **`SCHEMA_OUTPUT.md §8`**.
...
- **Não carregue essas linhas no Matrix**, e **avise o time responsável pela ferramenta informando o
  `ID Lançamento`** de cada uma (a coluna 16 da base; é a chave que permite achar a linha na base
...
**Grupo · Conta OM · Conta Contábil**, na aba **`De-Para Grupos`**. Seed inicial (os 11 grupos da v1):
...
input era lido pela **posição** (a 1ª aba do arquivo, `sheet: 0`); agora é lido pelo nome
`De-Para Grupos`, **caindo para a 1ª aba** quando esse nome não existe no arquivo — o fallback
preserva os arquivos de `depara_grupos` já em uso, cuja aba pode ter qualquer nome. O nome é
sobreponível por env var (`BBV001_DEPARA_GRUPOS_SHEET`). Consequência prática: **dá para enviar
...
lançamentos ficam no `base_final` com `Status = CADASTRO_PENDENTE`, `Motivo =
